...
with DAG(
    ...
    def get_daily_monitoring_rslt_task():
        ...
        with closing(postgres_hook.get_conn()) as conn:
            with closing(conn.cursor()) as cursor:
                with open("/opt/airflow/plugins/files/sqls/daily_dag_monitoring.sql", "r") as sql_file:
                    cursor.execute("SET TIME ZONE 'Asia/Seoul';")
                    sql = "\n".join(sql_file.readlines())
                    cursor.execute(sql)
                    rslt = cursor.fetchall()
                    rslt = pd.DataFrame(rslt)
                    rslt.columns = ["dag_id" , "run_cnt","success_cnt","failed_cnt","running_cunt","last_failed_date","last_success_date","next_dagrun_data_interval_start","next_dagrun_data_interval_end"]
                    return_blocks = []
    ...
    send_to_slack = SlackWebhookOperator(
        ...
        blocks= '{{ t1.xcom_pull(task_ids="get_daily_monitoring_rslt_task")}}'
    )
